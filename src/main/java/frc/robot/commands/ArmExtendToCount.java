...
public class ArmExtendToCount extends CommandBase {
  private Arm m_a;
  ...
  public ArmExtendToCount(Arm a, double c) {
    this.m_a = a;
    // as arm counts are always negative take absolute value to make compares easier
    this.m_targetCount = Math.abs(c);
    this.m_targetCountUL = this.m_targetCount  + (ArmParameters.k_rangeFactor)*(m_targetCount);
    if (this.m_targetCountUL > Math.abs(ArmParameters.k_fullExtendCount)) {
      this.m_targetCountUL = Math.abs(ArmParameters.k_fullExtendCount);
    }
    this.m_targetCountLL = this.m_targetCount  - (ArmParameters.k_rangeFactor)*(m_targetCount);
  ...
  public void execute() {
    ...
    double currentCount = Math.abs(m_a.getExtenderCount());
